...
public class SignController {
	...
	public String showSign(Model model) {
		...
		String redirectURL = "http://192.168.4.13:8080${contextPath}/callback/kakao"
								.replace("${contextPath}", application.getContextPath());
		
		kakaoLoginLink = kakaoLoginLink.replace("${client_id}", "23952a54f84ff853b8e5cda55c83230e");
		kakaoLoginLink = kakaoLoginLink.replace("${redirect_uri}", redirectURL);
...
//		
//		String redirectURL2 = "http://192.168.4.13:8080${contextPath }/callback/naver"
//				.replace("${contextPath}", application.getContextPath());
//		
//		naverLoginLink = naverLoginLink.replace("${client_id}", "lHVLEjFOMfMNSlJeg5dj");
//		naverLoginLink = naverLoginLink.replace("${redirect_uri}", redirectURL2);
	...
	public String acceptCode(@RequestParam String code, HttpSession session) {
		...
		if(found == null) {
			Account one = Account.builder().id(id).nickname(kakaoUserInfo.getProfile().getNickname())
									.platform("kakao").accessToken(oauthToken.getAccess_token())
	...
	public String acceptCodeForNaver(@RequestParam String code) {
		//억세스 토근 받아오기 (네이버)
